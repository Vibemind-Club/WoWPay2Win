# WoWPay2Win (Styka Sheets' hosted fork)

**This is a fork of [WoWPay2Win](https://github.com/Trinovantes/WoWPay2Win) by Trinovantes**,
served at <https://stykasheets.com/wowpay2win/> by [Styka Sheets](https://stykasheets.com/).
...
serves the SPA from this repo under the `/wowpay2win/` path. What the fork changes:
...
- **Split by secondary pair.** Raid BoEs roll a random pair of secondaries and the pair
  ...
  once per ORDERED pair - 12 per item, because the game lists the larger stat first and
  Crit / Vers is a different roll from Vers / Crit (`src/common/Boe.ts`, `BoeFilter.vue`,
  `useFilterStore.ts`). The filter key becomes `itemId:pair` (e.g. `271444:12` = Haste /
  Mastery, `271444:21` = Mastery / Haste) and rides the URL and localStorage the same way
  item ids did.
